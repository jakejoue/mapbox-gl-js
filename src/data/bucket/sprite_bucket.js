...
import {SpriteLayoutArray} from '../array_types';

import {members as layoutAttributes} from './sprite_attributes';
...
import {addPatternDependencies} from './pattern_bucket_features';
...
function addLineVertex(layoutVertexBuffer, point: Point, extrude: Point, round: boolean, up: boolean, dir: number, linesofar: number) {
    layoutVertexBuffer.emplaceBack(
		...
		255 & curOpacity, parseInt(curNum), parseInt(curStatus),
		parseInt(point.z === 0 ? 0 : point.z - 500 < 6 ? 6 : point.z - 500 + 6), parseInt(_percent));
}

/**
 * @private
 */
class SpriteBucket implements Bucket {
    ...
    zoom: number;
    overscaling: number;
    layers: Array<SpriteStyleLayer>;
    layerIds: Array<string>;
    stateDependentLayers: Array<any>;
    stateDependentLayerIds: Array<string>;
    features: Array<BucketFeature>;

    layoutVertexArray: SpriteLayoutArray;
    ...
    hasPattern: boolean;
    programConfigurations: ProgramConfigurationSet<SpriteStyleLayer>;
    segments: SegmentVector;
    uploaded: boolean;

    constructor(options: BucketParameters<SpriteStyleLayer>) {
        ...
        this.layoutVertexArray = new SpriteLayoutArray();
    ...
    addFeature(feature: BucketFeature, geometry: Array<Array<Point>>, index: number, canonical: CanonicalTileID, imagePositions: {[string]: ImagePosition}) {
        ...
        const join = layout.get('sprite-join').evaluate(feature, {});
        const cap = layout.get('sprite-cap');
        const miterLimit = layout.get('sprite-miter-limit');
        const roundLimit = layout.get('sprite-round-limit');

        curNum = feature.properties.link_seq ? feature.properties.link_seq : 0;
        curNum %= 10;
        curStatus = feature.properties.status || 1;
    ...
    addLine(vertices: Array<Point>, feature: BucketFeature, join: string, cap: string, miterLimit: number, roundLimit: number) {
        ...
        for (let i = first; i < len; i++) {
            ...
            curOpacity = Math.round(255 / len * i);
            ...
            // GeoGlobe liuck
            if (!prevNormal) {
                prevNormal = new Point(0, 0);
            }
            if (!nextNormal) {
                nextNormal = new Point(0, 0);
            }
...
register('SpriteBucket', SpriteBucket, {omit: ['layers', 'features']});

export default SpriteBucket;
